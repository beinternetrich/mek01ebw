...
class HTTP
{
	function __construct($host, $username, $password, $port=2082 , $ssl='' , $theme )
//	function __construct($host, $username, $password, $port = 2082, $ssl = '', $theme = 'x3')
	{
		$this->ssl = $ssl ? 'ssl://' : '';
		$this->username = $username;
		$this->password = urldecode($password);
		$this->theme = $theme;
		$this->auth = base64_encode($username . ':' . urldecode($password));
		$this->port = $port;
		$this->host = $host;
		$this->path = '/frontend/' . $theme . '/';
	}

	function ogetData($url, $data = '')
	{
		$url = $this->path . $url;
		if(is_array($data))
		{
	...
	function getData($url, $data = '')
	{
		...
		if(is_array($data))
		{
			...
			foreach($data as $key=>$value)
			{
			echo $url .= urlencode($key) . '=' . urlencode($value) . '&';
			}
			$url = substr($url, 0, -1);
		}
		$response = '';
...
//=================================
//**
		$url = 'http://'.$this->username.':'.$this->password.'@'.$_SERVER['HTTP_HOST'].$url; 
		$ch  = curl_init();
		//curl_setopt($ch, CURLOPT_USERAGENT, "PHP Script");
		curl_setopt($ch, CURLOPT_URL, $url);
		//curl_setopt($ch, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_0);
		curl_setopt($ch, CURLOPT_USERPWD,  $this->username.':'.$this->password);
		curl_setopt($ch, CURLOPT_HTTPAUTH, $this->auth);   //CURLAUTH_BASIC); //$this->auth);  //
...
class cPanel
{
  ...
  * @return cPanel
  */
	function __construct($host, $username, $password, $port , $ssl , $theme )
//	function __construct($host, $username, $password, $port = 2082, $ssl = false, $theme = 'x3')
  ...
  */
	...
	{
		$email = array();
		preg_match('/email" type=\"text\" checked value="(.*)"/', $this->HTTP->getData('contact/index.html'), $email);
		return $email[1];
	}
...
class emailAccount
{
  ...
  * @ignore
  */
	function __construct($host, $username, $password, $port = 2082, $ssl = false, $theme = 'x3', $address)
	{
		$this->HTTP = new HTTP($host, $username, $password, $port, $ssl, $theme);
		if(strpos($address, '@'))
		{
...
class Domain
{
  ...
  * @ignore
  */
	function __construct($host, $username, $password, $port = 2082, $ssl = false, $theme = 'x3', $domain)
	{
		$this->HTTP = new HTTP($host, $username, $password, $port, $ssl, $theme);
		$this->domain = $domain;
	}
...
class FTPAccount
{
  ...
  * @ignore
  */
	function __construct($host, $username, $password, $port = 2082, $ssl = false, $theme = 'x3', $account)
	{
		$this->HTTP = new HTTP($host, $username, $password, $port, $ssl, $theme);
...
class Database
{
  ...
  */
	...
	function __construct($host, $username, $password, $port = 2082, $ssl = false, $theme = 'x3', $database)
	{
		$this->HTTP = new HTTP($host, $username, $password, $port, $ssl, $theme);
...
class databaseUser
{
  ...
  * @ignore
  */
	function __construct($host, $username, $password, $port = 2082, $ssl = false, $theme = 'x3', $user)
	{
		$this->HTTP = new HTTP($host, $username, $password, $port, $ssl, $theme);
		$this->user = $user;
	}
...
class Redirect
{
  ...
  * @ignore
  */
	function __construct($host, $username, $password, $port = 2082, $ssl = false, $theme = 'x3', $path)
	{
		$this->HTTP = new HTTP($host, $username, $password, $port, $ssl, $theme);
		$this->path = '/' . $path;
	}
